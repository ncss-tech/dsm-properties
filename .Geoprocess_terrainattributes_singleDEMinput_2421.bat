...
REM REM ####### Compress files and convert to .tif
REM REM %%x is the iterator of the full file name
REM REM %%~nx just means the file name without the extension. 
REM setlocal enabledelayedexpansion enableextensions
REM for /f %%x in ('dir /b *.sdat') do (
REM echo now converting and compressing %%x 
REM gdalwarp -co COMPRESS=DEFLATE -co PREDICTOR=2 -co TILED=YES %%x %desFol%%%~nx.tif
REM )

REM REM Remove intermediate saga grid files since I only want geotiffs.
REM setlocal enabledelayedexpansion enableextensions
REM for /f %%x in ('dir /b *.sdat') do (
REM echo now removing %%x
REM rm %%x
REM )

REM setlocal enabledelayedexpansion enableextensions
REM for /f %%x in ('dir /b *.mgrd') do (
REM echo now removing %%x
REM rm %%x
REM )

REM setlocal enabledelayedexpansion enableextensions
REM for /f %%x in ('dir /b *.sgrd') do (
REM echo now removing %%x
REM rm %%x
REM )

REM setlocal enabledelayedexpansion enableextensions
REM for /f %%x in ('dir /b *.sdat.aux.xml') do (
REM echo now removing %%x
REM rm %%x
REM )

REM setlocal enabledelayedexpansion enableextensions
REM for /f %%x in ('dir /b *.prj') do (
REM echo now removing %%x
REM rm %%x
REM )
